...
class MaskedMSELoss(nn.Module):
    """Calculates Mean Squared Error exclusively on ocean pixels (mask == 1)."""
    def __init__(self):
        super().__init__()

    def forward(self, pred: torch.Tensor, target: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        # Expand 2D ocean mask (B, H, W) to match 3D depth dimensions (B, Depth, H, W)
...
if __name__ == "__main__":
    ...
    dummy_mask = torch.ones(4, 101, 241)

    output = model(dummy_input)
    criterion = MaskedMSELoss()
    loss = criterion(output, dummy_target, dummy_mask)
